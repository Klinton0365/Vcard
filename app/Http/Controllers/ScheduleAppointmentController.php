...
use Modules\MercadoPago\App\Http\Controllers\UserMercadoPagoController;
...
class ScheduleAppointmentController extends AppBaseController
{
        ...
        try {
            ...
            if (isset($input['payment_method'])) {
                ...
                if ($input['payment_method'] == Appointment::PAYSTACK) {
                    ...
                    if (isset($input['currency_code']) && ! in_array(
                        strtoupper($input['currency_code']),
                ...
                if ($input['payment_method'] == Appointment::MERCADOPAGO) {
                    ...
                    $result = App::make(UserMercadoPagoController::class)->appointmentOnBoard($userId, $vcard, $input);
                ...
                // Razor Pay
                if ($input['payment_method'] == Appointment::RAZORPAY) {

                    $repo = App::make(AppointmentRepository::class);

                    $result = $repo->userCreateRazorPaySession($input, $userId);
                    $result['payment_method'] = $input['payment_method'];
                    $userId = $userId;

                    session()->put(['appointment_details' => $input]);
                    Session::put(['vcard_user_id' => $userId, 'alias' => $vcard->url_alias, 'vcard_id' => $vcard->id, 'tenant_id' => $vcard->tenant->id]);
                    DB::commit();
                    $result['key'] = getUserSettingValue('razorpay_key', $userId);
    ...
    public function getCalendar()
    {
        ...
        foreach ($appointments as $key => $appointment) {
            ...
            if (getUserSettingValue('time_format', getLogInUserId()) == UserSetting::HOUR_24) {
                ...
                $data[$key]['title'] = date('H:i', strtotime($appointment->from_time)) . '-' . date(
                    'H:i',
    ...
    public function appointmentsUpdate(ScheduleAppointment $appointment)
    {
        ...
        $data['date'] = Carbon::parse($appointment->date)->format('d M, Y');
