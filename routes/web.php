...
Route::middleware(['freshInstall'])->group(function () {
    ...
    Route::prefix('admin')->middleware('subscription', 'auth', 'valid.user', 'role:admin', 'multi_tenant')->group(function () {
		...
		Route::get('/deepseek-chats', [DeepSeekController::class, 'chatIndex'])->name('deepseek.index');
		Route::post('/deepseek-chat', [DeepSeekController::class, 'chat'])->name('deepseeks');
		
		// Add this new route for AI description generation
    Route::post('/generate-description', [DeepSeekController::class, 'generateDescription'])->name('vcards.generate-description');
    ...
    //whatsapp stores  URL
    Route::get('whatsapp-store/{alias}', [WhatsappStoreController::class, 'show'])->name('whatsapp.store.show')->middleware('language');
